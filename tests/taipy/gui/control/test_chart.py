...
def test_chart_md_1(gui: Gui, helpers, csvdata):
    ...
    md_string = "<|{csvdata}|chart|x=Day|selected_color=green|y[1]=Daily hospital occupancy|label[1]=Entity|y[2]=Daily hospital occupancy|label[2]=Code|mode[2]=markers|color[2]=red|type[2]=scatter|xaxis[2]=x2|layout={subplot_layout}|width=100%|height=100%|selected={selected_indices}|>"
    expected_list = [
        "<Chart",
...
def test_chart_html_1(gui: Gui, helpers, csvdata):
    selected_indices = [14258]
    subplot_layout = {"grid": {"rows": 1, "columns": 2, "subplots": [["xy", "x2y"]], "roworder": "bottom to top"}}
    html_string = '<taipy:chart data="{csvdata}" x="Day" selected_color="green" y[1]="Daily hospital occupancy" label[1]="Entity" y[2]="Daily hospital occupancy" label[2]="Code" mode[2]="markers" color[2]="red" type[2]="scatter" xaxis[2]="x2" layout="{subplot_layout}" width="100%" height="100%" selected="{selected_indices}"  />'
    expected_list = [
        "<Chart",
        "selected0={tpec_TpExPr_selected_indices_TPMDL_0}",
        "selected1={tpec_TpExPr_selected_indices_TPMDL_0}",
...
def test_chart_html_2(gui: Gui, helpers, csvdata):
    selected_indices = [14258]
    subplot_layout = {"grid": {"rows": 1, "columns": 2, "subplots": [["xy", "x2y"]], "roworder": "bottom to top"}}
    html_string = '<taipy:chart x="Day" selected_color="green" y[1]="Daily hospital occupancy" label[1]="Entity" y[2]="Daily hospital occupancy" label[2]="Code" mode[2]="markers" color[2]="red" type[2]="scatter" xaxis[2]="x2" layout="{subplot_layout}" width="100%" height="100%" selected="{selected_indices}" >{csvdata}</taipy:chart>'
    expected_list = [
        "<Chart",
        "selected0={tpec_TpExPr_selected_indices_TPMDL_0}",
        "selected1={tpec_TpExPr_selected_indices_TPMDL_0}",
