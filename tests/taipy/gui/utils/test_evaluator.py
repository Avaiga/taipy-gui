...
def test_evaluate_expressions_same_variable(gui: Gui):
    ...
    with gui.get_flask_app().app_context():
        ...
        assert "tp_TaIpY_EXPR_x" in s1 and "tp_TaIpY_EXPR_x" in s2
...
def test_evaluate_holder(gui: Gui, test_client):
    ...
    with warnings.catch_warnings(record=True) as w:
        with gui.get_flask_app().app_context():
            ...
            hash = gui._evaluate_bind_holder(_TaipyNumber, "TaIpY_EXPR_x + 10_TPMDL_0")
            assert "_TpN_tp_TaIpY_EXPR_x_10_TPMDL_0_0" in hash
            lst = gui._evaluate_holders("TaIpY_EXPR_x + 10_TPMDL_0")
            assert len(lst) == 1
            assert "_TpN_tp_TaIpY_EXPR_x_10_TPMDL_0_0" in lst[0]
