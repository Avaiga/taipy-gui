...
class Gui:
    ...
    def __init__(
        ...
        try:
            ...
            warnings_warn(f"Cannot retrieve version.json file:\n{e}")
            self.__version = {}
    ...
    def _manage_message(self, msg_type: _WsType, message: dict) -> None:
        try:
            ...
            warnings_warn(f"Decoding Message has failed: {message}\n{e}")
        ...
        if callable(on_change_fn):
            try:
                ...
                if not self._call_on_exception(on_change or "on_change", e):
                    warnings_warn(f"{on_change or 'on_change'}: callback function raised an exception:\n{e}")
    ...
    def __serve_user_content(self, path: str) -> t.Any:
        if hasattr(self, "on_user_content") and callable(self.on_user_content):
            ...
            try:
                ...
                if ret is None:
                    warnings_warn("on_user_content() callback function should return a value.")
                else:
                    return (ret, 200)
            except Exception as e:  # pragma: no cover
                if not self._call_on_exception("on_user_content", e):
                    warnings_warn(f"on_user_content() callback function raised an exception:\n{e}")
        else:
            warnings_warn("on_user_content() callback function has not been defined.")
        return ("", 404)
    ...
    def _serve_status(self, template: pathlib.Path) -> t.Dict[str, t.Dict[str, str]]:
        ...
        if self._get_config("extended_status", False):
            ...
            try:
                ...
                warnings_warn(f"Exception raised in json reading in '{template}':\n{e}")
        return {"gui": base_json}

    def __upload_files(self):
        self.__set_client_id_in_context()
        if "var_name" not in request.form:
            warnings_warn("No var name")
        ...
        if "blob" not in request.files:
            warnings_warn("No file part")
            return ("No file part", 400)
        file = request.files["blob"]
        # If the user does not select a file, the browser submits an
        # empty file without a filename.
        if file.filename == "":
            warnings_warn("No selected file")
            ...
            if complete:
                if part > 0:
                    try:
                        ...
                        warnings_warn(f"cannot group file after chunk upload:\n{ee}")
                        return
    ...
    def __send_ws(self, payload: dict) -> None:
        ...
        if grouping_message is None:
            try:
                ...
                warnings_warn(f"Exception raised in Web Socket communication in '{self.__frame.f_code.co_name}':\n{e}")
        else:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        try:
            ...
            warnings_warn(f"Exception raised while sending messages:\n{e}")
        if exc_value:  # pragma: no cover
            warnings_warn(f"An {exc_type or 'Exception'} was raised: {exc_value}")
        return True
    ...
    def __on_action(self, id: t.Optional[str], payload: t.Any) -> None:
        ...
        if action:
            if self.__call_function_with_args(
                ...
                warnings_warn(f"on_action: '{action}' is not a valid function")
        ...
        if callable(action_function):
            try:
                ...
                if not self._call_on_exception(action_function.__name__, e):
                    warnings_warn(f"on_action: Exception raised in function '{action_function.__name__}':\n{e}")
        return False
    ...
    def _call_user_callback(
        ...
        try:
            ...
            if not self._call_on_exception(user_callback.__name__, e):
                warnings_warn(f"invoke_callback: Exception raised in function '{user_callback.__name__}'.\n{e}")
        return None
    ...
    def _tbl_cols(
        ...
        if not self.__is_building():
            try:
                ...
                warnings_warn(f"Exception while rebuilding table columns {e}")
    ...
    def _chart_conf(
        ...
        if not self.__is_building():
            try:
                ...
                warnings_warn(f"Exception while rebuilding chart config {e}")
        return Gui.__DO_NOT_UPDATE_VALUE
    ...
    def add_partial(
        ...
        if (
            ...
            warnings_warn(f'Partial name "{new_partial._route}" is already defined.')
        if isinstance(page, str):
            from .renderers import Markdown
    ...
    def __bind_local_func(self, name: str):
        ...
        if func is not None:
            if callable(func):
                ...
                warnings_warn(f"{name}: {func} should be a function.")
    ...
    def _navigate(self, to: t.Optional[str] = "", tab: t.Optional[str] = None):
        ...
        if not to.startswith("/") and to not in self._config.routes and not urlparse(to).netloc:
            warnings_warn(f'Cannot navigate to "{to if to != Gui.__root_page_name else "/"}": unknown page.')
            return False
        ...
        if hasattr(self, "on_init") and callable(self.on_init) and not _hasscopeattr(self, Gui.__ON_INIT_NAME):
            ...
            try:
                ...
                if not self._call_on_exception("on_init", e):
                    warnings_warn(f"Exception raised in on_init.\n{e}")
            ...
            for lib in libs:
                ...
                try:
                    ...
                    if not self._call_on_exception(f"{name}.on_user_init", e):
                        warnings_warn(f"Exception raised in {name}.on_user_init().\n{e}")
    ...
    def _call_on_exception(self, function_name: str, exception: Exception) -> bool:
        if hasattr(self, "on_exception") and callable(self.on_exception):
            try:
                ...
                warnings_warn(f"Exception raised in on_exception.\n{e}")
            return True
        return False
        ...
        if hasattr(self, "on_status") and callable(self.on_status):
            try:
                ...
                if not self._call_on_exception("on_status", e):
                    warnings_warn(f"Exception raised in on_status.\n{e}")
        return None
            ...
            try:
                ...
                if nav_page != page_name:
                    if isinstance(nav_page, str):
                        ...
                        warnings_warn(f"on_navigate() returned an invalid page name '{nav_page}'.")
                    nav_page = page_name
            except Exception as e:  # pragma: no cover
                if not self._call_on_exception("on_navigate", e):
                    warnings_warn(f"Exception raised in on_navigate.\n{e}")
    ...
    def __register_blueprint(self):
        ...
        if _conf_webapp_path:
            if _conf_webapp_path.is_dir():
                ...
                warnings_warn(f"Now using webapp_path: '{_conf_webapp_path}'.")
            else:  # pragma: no cover
                warnings_warn(
            ...
            for lib in libs:
                ...
                try:
                    ...
                    if not self._call_on_exception(f"{name}.on_init", e):
                        warnings_warn(f"Method {name}.on_init() raised an exception:\n{e}.")
