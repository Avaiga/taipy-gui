...
def _update_col_desc_from_indexed(
    ...
    for k, v in col_value.items():
        if col_desc := next((x for x in columns.values() if x.get("dfid") == k), None):
            ...
            warnings_warn(f"{elt_name} {name}[{k}] is not in the list of displayed columns")
    ...
    for k, v in filters.items():
        if _is_boolean_true(v):
            if col_desc := _get_column_desc(columns, k):
                ...
                warnings_warn(f"{elt_name} filter[{k}] is not in the list of displayed columns")
    ...
    for k, v in editables.items():
        if _is_boolean(v):
            if col_desc := _get_column_desc(columns, k):
                ...
                warnings_warn(f"{elt_name} editable[{k}] is not in the list of displayed columns")
    ...
    for k, v in group_by.items():
        if _is_boolean_true(v):
            if col_desc := _get_column_desc(columns, k):
                ...
                warnings_warn(f"{elt_name} group_by[{k}] is not in the list of displayed columns")
    ...
    for k, v in apply.items():  # pragma: no cover
        if col_desc := _get_column_desc(columns, k):
            if callable(v):
                ...
                warnings_warn(f"{elt_name} apply[{k}] should be a user or predefined function")
                value = None
            if value:
                col_desc["apply"] = value
        else:
            warnings_warn(f"{elt_name} apply[{k}] is not in the list of displayed columns")
    ...
    for k, v in styles.items():  # pragma: no cover
        if col_desc := _get_column_desc(columns, k):
            ...
            if value in columns.keys():
                warnings_warn(f"{elt_name} style[{k}]={value} cannot be a column's name")
            elif value:
                col_desc["style"] = value
        else:
            warnings_warn(f"{elt_name} style[{k}] is not in the list of displayed columns")
    ...
    for k, v in tooltips.items():  # pragma: no cover
        if col_desc := _get_column_desc(columns, k):
            if callable(v):
                ...
                value = None
            if value in columns.keys():
                warnings_warn(f"{elt_name} tooltip[{k}]={value} cannot be a column's name")
            elif value:
                col_desc["tooltip"] = value
        else:
            warnings_warn(f"{elt_name} tooltip[{k}] is not in the list of displayed columns")
    return columns
