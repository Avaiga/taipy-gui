...
class _Builder:
    ...
    def __init__(
        ...
        if "properties" in self.__attributes:
            ...
            if isinstance(prop_dict, _MapDict):
                ...
                warnings_warn(f"{self.__control_type}.properties ({prop_hash}) must be a dict.")
    ...
    def __parse_attribute_value(gui: "Gui", value) -> t.Tuple:
        if isinstance(value, str) and gui._is_expression(value):
            ...
            try:
                ...
                warnings_warn(f"Expression '{value}' cannot be evaluated")
        return (value, None)
    ...
    def set_dict_attribute(self, name: str, default_value: t.Optional[t.Dict[str, t.Any]] = None):
        ...
        if dict_attr is not None:
            ...
            if isinstance(dict_attr, (dict, _MapDict)):
                ...
                warnings_warn(f"{self.__element_name} {name} should be a dict\n'{str(dict_attr)}'")
        return self
    ...
    def __set_list_of_(self, name: str):
        ...
        if not isinstance(lof, (list, tuple)):
            if lof is not None:
                warnings_warn(f"{self.__element_name} {name} should be a list")
            return self
    ...
    def set_number_attribute(self, name: str, default_value: t.Optional[str] = None, optional: t.Optional[bool] = True):
        ...
        if value is None:
            if not optional:
                warnings_warn(f"Property {name} is required for control {self.__control_type}")
            return self
    ...
    def __set_function_attribute(
        ...
        if strattr is None:
            ...
            if func == strattr:
                warnings_warn(f" {self.__control_type}.{name}: {strattr} is not a function")
    ...
    def _get_adapter(self, var_name: str, property_name: t.Optional[str] = None, multi_selection=True):  # noqa: C901
        ...
        if isinstance(lov, list):
            ...
            if adapter and not callable(adapter):
                warnings_warn("'adapter' property value is invalid")
                adapter = None
    ...
    def _get_dataframe_attributes(self) -> "_Builder":
        ...
        if line_style := self.__attributes.get("style"):
            ...
            if value in col_types.keys():
                warnings_warn(f"{self.__element_name} style={value} cannot be a column's name")
            elif value:
        ...
        if tooltip := self.__attributes.get("tooltip"):
            if callable(tooltip):
                ...
                value = None
            if value in col_types.keys():
                warnings_warn(f"{self.__element_name} tooltip={value} cannot be a column's name")
            elif value:
    ...
    def _set_string_with_check(self, var_name: str, values: t.List[str], default_value: t.Optional[str] = None):
        ...
        if value is not None:
            ...
            if value not in values:
                warnings_warn(f"{self.__element_name} {var_name}={value} should be in {values}")
            else:
    ...
    def __set_list_attribute(
        ...
        if isinstance(val, list):
            ...
            warnings_warn(f"{self.__element_name} {name} should be a list of {elt_type}")
        return []
    ...
    def _set_file_content(self, var_name: str = "content"):
        if hash_name := self.__hashes.get(var_name):
            ...
            warnings_warn("{self.element_name} {var_name} should be bound")
        return self
    ...
    def __set_dynamic_number_attribute(self, var_name: str, default_value: t.Any):
        ...
        if isinstance(numVal, str):
            try:
                ...
                warnings_warn(f"{self.__element_name} {var_name} cannot be transformed into a number\n{e}")
                numVal = 0
        if isinstance(numVal, numbers.Number):
            ...
            warnings_warn(f"{self.__element_name} {var_name} value is not not valid {numVal}")
        if hash_name:
    ...
    def _set_table_pagesize_options(self, default_size=[50, 100, 500]):
        ...
        if isinstance(page_size_options, str):
            try:
                ...
                warnings_warn(f"{self.__element_name} page_size_options: invalid value {page_size_options}\n{e}")
        if isinstance(page_size_options, list):
            self.__set_json_attribute("pageSizeOptions", page_size_options)
        else:
            warnings_warn(f"{self.__element_name} page_size_options should be a list")
        return self
    ...
    def __set_dynamic_property_without_default(self, name: str, property_type: PropertyType):
        ...
        if hash_name is None:
            warnings_warn(f"{self.__element_name}.{name} should be bound.")
        else:
