...
class _Preprocessor(MdPreprocessor):
    ...
    def run(self, lines: List[str]) -> List[str]:
        ...
        for line_count, line in enumerate(lines, start=1):
            ...
            if m is not None:
                ...
                if tag in _MarkdownFactory._TAIPY_BLOCK_TAGS:
                    ...
                    warnings_warn(f"Invalid tag name '{tag}' in line {line_count}")
            # Other controls
            ...
            if m is not None:
                if len(tag_queue):
                    ...
                    warnings_warn(f"Line {line_count} has an unmatched closing tag")
            # append the new line
        ...
        for tag, line_no, _ in tag_queue:
            ...
            warnings_warn(f"Opened tag {tag} in line {line_no} is not closed")
        return new_lines
    ...
    def _process_control(
        ...
        for fragment in fragments:
            if control_name is None and _MarkdownFactory.get_default_property_name(fragment):
                ...
                if not_prefix and val:
                    warnings_warn(f"Negated property {prop_name} value ignored at {line_count}")
                prop_value = "True"
                if not_prefix:
                    prop_value = "False"
                elif val:
                ...
                warnings_warn(f"Bad Taipy property format at line {line_count}: '{fragment}'")
        if control_name is None:
            if properties and all(attribute != properties[0][0] for attribute in _MarkdownFactory._TEXT_ATTRIBUTES):
                ...
                warnings_warn(f"Unrecognized control {control_name} at line {line_count}: <|{prop_string}|>")
            else:
