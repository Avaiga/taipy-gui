...
def download(state: State, content: t.Any, name: t.Optional[str] = "", on_action: t.Optional[str] = ""):
    ...
    if state and isinstance(state._gui, Gui):
        ...
        warnings_warn("'download()' must be called in the context of a callback")
...
def notify(
    ...
    if state and isinstance(state._gui, Gui):
        ...
        warnings_warn("'notify()' must be called in the context of a callback")
...
def hold_control(
    ...
    if state and isinstance(state._gui, Gui):
        ...
        warnings_warn("'hold_actions()' must be called in the context of a callback")
...
def resume_control(state: State):
    ...
    if state and isinstance(state._gui, Gui):
        ...
        warnings_warn("'resume_actions()' must be called in the context of a callback")
...
def navigate(state: State, to: t.Optional[str] = "", tab: t.Optional[str] = None):
    ...
    if state and isinstance(state._gui, Gui):
        ...
        warnings_warn("'navigate()' must be called in the context of a callback")
...
def get_user_content_url(
    ...
    warnings_warn("'get_user_content_url()' must be called in the context of a callback")
    return None
...
def get_context_id(state: State) -> t.Any:
    warnings_warn("'get_context_id()' was deprecated in Taipy GUI 2.0. Use 'get_state_id()' instead.")
...
def invoke_callback(
    ...
    warnings_warn("'invoke_callback()' must be called with a valid Gui instance")


def invoke_state_callback(gui: Gui, state_id: str, callback: t.Callable, args: t.Union[t.Tuple, t.List]) -> t.Any:
    warnings_warn("'invoke_state_callback()' was deprecated in Taipy GUI 2.0. Use 'invoke_callback()' instead.")
...
def invoke_long_callback(
    ...
    *user_function* is finished (successfully or not), or periodically (using the *period* parameter).
    ...
    if not state or not isinstance(state._gui, Gui):
        warnings_warn("'invoke_long_callback()' must be called in the context of a callback.")
        return
    ...
    def callback_on_exception(state: State, function_name: str, e: Exception):
        if not this_gui._call_on_exception(function_name, e):
            warnings_warn(f"invoke_long_callback: Exception raised in function {function_name}.\n{e}")
