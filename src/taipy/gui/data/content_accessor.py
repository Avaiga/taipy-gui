    ...
    def __get_mime_from_file(self, path: pathlib.Path):
        if _has_magic_module:
            try:
                ...
                warnings_warn(f"({path}) cannot read mime type.\n{e}")
        return None
        ...
        if not isinstance(newvalue, (str, pathlib.Path)) and (
            ...
            if _has_magic_module:
                try:
                    ...
                    warnings_warn(f"{self.__get_display_name(var_name)} ({type(value)}) cannot be typed.\n{e}")
        ...
        if isinstance(newvalue, (str, pathlib.Path)):
            ...
            if not path.is_file():
                if (
                    ...
                    warnings_warn(f"{self.__get_display_name(var_name)} ({value}) file does not exist.")
                return str(value)
            if image:
                if not mime:
                    mime = self.__get_mime_from_file(path)
                if mime and not mime.startswith("image"):
                    warnings_warn(f"{self.__get_display_name(var_name)} ({path}) is not an image: {mime}")
            ...
            try:
                ...
                warnings_warn(f"{self.__get_display_name(var_name)} ({type(value)}) is not an image: {mime}")
                return f"Invalid content: {mime}"
            except Exception as e:
                warnings_warn(f"{self.__get_display_name(var_name)} ({type(value)}) cannot be base64 encoded.\n{e}")
