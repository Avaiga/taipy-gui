...
class _PandasDataAccessor(_DataAccessor):
    ...
    def __user_function(
        ...
        try:
            ...
            warnings_warn(f"Exception raised when calling user function {function_name}\n{e}")
        return ""
    ...
    def __apply_user_function(
        ...
        try:
            ...
            warnings_warn(f"Exception raised when invoking user function {function_name}.\n{e}")
        return False
    ...
    def __get_data(  # noqa: C901
        ...
        if isinstance(filters, list) and len(filters) > 0:
            ...
            try:
                ...
                warnings_warn(f"Dataframe filtering: invalid query '{query}' on {value.head()}\n{e}")
        ...
        if paged:
            ...
            if isinstance(aggregates, list) and len(aggregates) and isinstance(applies, dict):
                ...
                try:
                    ...
                    warnings_warn(f"Cannot aggregate {var_name} with groupby {aggregates} and aggregates {applies}")
            ...
            if isinstance(payload["start"], int):
                ...
                try:
                    ...
                    warnings_warn(f'start should be an int value {payload["start"]}')
                    start = 0
            ...
            if isinstance(order_by, str) and len(order_by):
                try:
                    ...
                    warnings_warn(f"Cannot sort {var_name} on columns {order_by}.")
                ...
                if isinstance(decimator_instance, PropertyType.decimator.value):
                    ...
                    if nb_rows_max and decimator_instance._is_applicable(value, nb_rows_max, chart_mode):
                        try:
                            ...
                            warnings_warn(f"Limit rows error with {decimator} for dataframe: {e}")
