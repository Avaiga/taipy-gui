...
class Gui:
    ...
    def run(self, run_server: bool = True, run_in_thread: bool = False, **kwargs) -> None:
        ...
        self.__frame = t.cast(FrameType, kwargs.pop("_frame", None) or inspect.currentframe().f_back)
        if not isinstance(self.__frame, FrameType):
            raise RuntimeError("_frame must be a FrameType where Taipy.Gui can collect the local variables.")
